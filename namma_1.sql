...
select * from
(select *,dense_rank() over(order by total_trips desc) rnk
from 
(select loc_from,loc_to,COUNT(tripid) as total_trips from trips
...
group by loc_from,duration)b)c
where rnk=1
